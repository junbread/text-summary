...
from .layers import Encoder, Decoder, Attention, Pointer


class PGNModel(tf.keras.Model):
    def __init__(self, params):
        super(PGNModel, self).__init__()

        self.params = params
        self.encoder = Encoder(
            params["vocab_size"], params["embed_size"], params["enc_units"], params["batch_size"])
        self.attention = Attention(params["attn_units"])
        self.decoder = Decoder(
            params["vocab_size"], params["embed_size"], params["dec_units"], params["batch_size"])
    ...
    def call(self, enc_output, dec_hidden, enc_input, enc_input_extended, dec_inp, batch_oov_len):
        predictions = []
        attentions = []
        p_gens = []
        ...
        for t in range(dec_inp.shape[1]):
            dec_x, pred, dec_hidden = self.decoder(tf.expand_dims(
                dec_inp[:, t], 1), dec_hidden, enc_output, context_vector)
        ...
        final_dists = _calc_final_dist(enc_input_extended, predictions, attentions,
