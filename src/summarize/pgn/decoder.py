...
def beam_decode(model, batch, vocab, params):
    ...
    def decode_onestep(batch, enc_outputs, dec_state, dec_input):
        ...
        final_dists, dec_hidden, context_vector, attentions, p_gens = model(
            enc_outputs, dec_state, batch[0]["enc_input"], batch[0]["extended_enc_input"], dec_input, batch[0]["max_oov_len"])
    ...
    while steps < params['max_dec_steps'] and len(results) < params['beam_size']:
        ...
        latest_tokens = [t if t in range(params['vocab_size']) else vocab.word_to_id(
            '[UNK]') for t in latest_tokens]  # we replace all the oov is by the unknown token
        ...
        returns = decode_onestep(batch, enc_outputs, tf.stack(
            states, axis=0), tf.expand_dims(latest_tokens, axis=1))
        topk_ids, topk_log_probs, new_states, attn_dists, p_gens = returns['top_k_ids'], returns[
            'top_k_log_probs'], returns['dec_state'], returns['attention_vec'], np.squeeze(returns["p_gen"])
        all_hyps = []
        num_orig_hyps = 1 if steps == 0 else len(hyps)
        for i in range(num_orig_hyps):
            h, new_state, attn_dist, p_gen = hyps[i], new_states[i], attn_dists[i], p_gens[i] if len(
                p_gens.shape) else p_gens
    ...
    best_hyp.abstract = " ".join(data_helper.output_to_words(
        best_hyp.tokens, vocab, batch[0]["article_oovs"][0])[1:-1])
